 ...
 *
 *****************************************************************************/
#ifndef HYBRID_NEWTON_RAPHSON_H
#define HYBRID_NEWTON_RAPHSON_H
...
public:
  HYBRID_NEWTON_RAPHSON(INTEGRATOR* integrator, SOLVER* solver):
  ...
  bool run()
  {
    ...
    for(; iteration < _maxNewtonIterations; iteration++){
      ...
      if(_integrator->activeFullDofs() > 0){
        TIMING_BREAKDOWN::tic();
        bool converged = _solver->solve(_integrator->gradient(), _delta);
        ...
        TIMING_BREAKDOWN::tic();
        _integrator->setPosition(_delta);
        TIMING_BREAKDOWN::toc("Set Position"); 
      }else{
        _integrator->computeQi(_integrator->gradient());
        TIMING_BREAKDOWN::tic();
        _integrator->setPosition(_delta);
        TIMING_BREAKDOWN::toc("Set Position");
      }
