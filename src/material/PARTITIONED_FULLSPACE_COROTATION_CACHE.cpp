...
VECTOR& PARTITIONED_FULLSPACE_COROTATION_CACHE::computePartialSurfaceInternalForce()
{
  TIMING_BREAKDOWN::tic();

  VECTOR& R = _tetMesh->partitionedSurfaceInternalForce();
  if(R.size() != _tetMesh->partitionFullsimDofStartIdx(_tetMesh->totalPartitions()))
    R.resize(_tetMesh->partitionFullsimDofStartIdx(_tetMesh->totalPartitions()));
    ...
    for(unsigned int x = 0; x < _tetMesh->totalPartitions(); x++){
      ...
      for(unsigned int y = 0; y < tetIDs.size(); y++){
        ...
        for(int z = 0; z < 4; z++){
          ...
          if(index * 3 < _tetMesh->partitionedFullsimDofs(x)){
            R.segment<3>(_tetMesh->partitionFullsimDofStartIdx(x) + index * 3) += forces.segment<3>(z * 3);
          }
        }
      }
    }
...
VECTOR& PARTITIONED_FULLSPACE_COROTATION_CACHE::computeSurfaceInternalForce()
{
  TIMING_BREAKDOWN::tic();

  VECTOR& R = _tetMesh->partitionedSurfaceInternalForce();
  if(R.size() != _tetMesh->partitionedSurfaceDofs())
    R.resize(_tetMesh->partitionedSurfaceDofs());
...
#pragma omp parallel
  ...
  { 
    ...
    #endif
    for(unsigned int x = 0; x < _tetMesh->totalPartitions(); x++){
      vector<int>& tetIDs = _tetMesh->partitionedSurfaceTetIDs(x);
      ...
      for(unsigned int y = 0; y < tetIDs.size(); y++){
        ...
        for(int z = 0; z < 4; z++){
          int originalVertexID = _tetMesh->vertexID(tet.vertices[z]);
          if(_tetMesh->isSurfaceVertex(originalVertexID)){
            int index = partitionedVertexIDs[originalVertexID];
            R.segment<3>(_tetMesh->partitionSurfaceDofStartIdx(x) + index * 3) += forces.segment<3>(z * 3);
          }
        }
      }
    }
...
void PARTITIONED_FULLSPACE_COROTATION_CACHE::computeSurfaceStiffnessMatrix(int partition, COO_MATRIX& diagMat, COO_MATRIX& offDiagMat)
{
  vector<TET>& tets = _tetMesh->tets();
  int surfaceDofs = _tetMesh->partitionedSurfaceVertexSize(partition) * 3;
  int internalDofs = _tetMesh->partitionedVertices(partition).size() * 3 - surfaceDofs;

  diagMat.resize(surfaceDofs, surfaceDofs);
  offDiagMat.resize(internalDofs, surfaceDofs);

  vector<int>& surfaceTetIDs = _tetMesh->partitionedSurfaceTetIDs(partition);
...
#pragma omp parallel
  ...
  COO_MATRIX diagLocalCooStiffness(surfaceDofs, surfaceDofs);
  COO_MATRIX offDiagLocalCooStiffness(internalDofs, surfaceDofs);
  ...
  for (unsigned int x = 0; x < surfaceTetIDs.size(); x++)
  {
    ...
    for (int y = 0; y < 12; y++)
    {
      ...
      if(partitionedVertexID >= surfaceDofs / 3)
          continue;
...
void PARTITIONED_FULLSPACE_COROTATION_CACHE::computeSurfaceStiffnessMatrices()
{
  TIMING_BREAKDOWN::tic();

  BLOCK_SPARSE_MATRIX& diagPart = _tetMesh->partitionedFullStiffnessDiag();
  diagPart.clear();

  BLOCK_SPARSE_MATRIX& offDiagPart = _tetMesh->partitionedFullStiffnessOffDiag();
  offDiagPart.clear();

  for(int x = 0; x < _tetMesh->totalPartitions(); x++){
    COO_MATRIX diagMat;
    COO_MATRIX offDiagMat;
    computeSurfaceStiffnessMatrix(x, diagMat, offDiagMat);
    diagPart.equals(diagMat, x, x);
    offDiagPart.equals(offDiagMat, x, x);
  }
}
